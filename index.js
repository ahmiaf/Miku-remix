...
console.log(color('\nHello, I am Ahmi, the main developer of this bot.\n\nThanks for using: Miku Bot','aqua'))
console.log(color('\nYou can follow me on GitHub: Ahmiaf','aqua'))

    let { version, isLatest } = await fetchLatestBaileysVersion()
    const Miku = MikuConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Miku by: Ahmi','Safari','1.0.0'],
        auth: state,
        version
    })
...
store.bind(Miku.ev)
    ...
    Miku.sendMessage(callerId, { text: `Fool! You will be blocked automatically for calling me!`}, { quoted : pa7rick })
    await sleep(8000)
...
*/
    ...
    Miku.ev.on('groups.update', async pea => {
     ...
     // Get Profile Picture Group
        ...
        let wm_fatih = { url : ppgc }
        if (pea[0].announce == true) {
        //Miku.send5ButImg(pea[0].id, `Group has been *Closed!* Only *Admins* can send Messages!`, `Miku Bot`, wm_fatih, [])
